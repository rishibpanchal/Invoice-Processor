"""
Batch Invoice Processing Script - Multi-Stage Approach
Process multiple PDF invoices using multi-stage extraction and export results
"""
import os
import sys
import json
...
try:
    ...
    print(f"❌ Error importing enhanced_multi_stage_extractor: {e}")
    print("Make sure all dependencies are installed and Ollama is running")
    sys.exit(1)


def process_single_invoice(extractor: EnhancedMultiStageExtractor, pdf_path: Path) -> Dict[str, Any]:
    """Process a single invoice using multi-stage extraction and return results"""
    print(f"📄 Processing: {pdf_path.name}")
    
    try:
        result = extractor.extract_data_with_stages(str(pdf_path))
...
def batch_process_invoices(input_dir: str, output_dir: str = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print("✅ Multi-stage extractor initialized")
    except Exception as e:
        print(f"❌ Failed to initialize multi-stage extractor: {e}")
        return []
...
def create_consolidated_csv(results: List[Dict[str, Any]], output_file: Path):
    """Create a consolidated CSV file with all line items from multi-stage results"""
    try:
        all_line_items = []
        ...
        for result in results:
            ...
            # Handle multi-stage structure with multiple invoices
            invoices = result.get('invoices', [])
            if not invoices:
                # Add empty row if no invoices found
                all_line_items.append({
                    'source_file': source_file,
                    'invoice_number': 'N/A',
                    'vendor_name': 'N/A',
                    'invoice_date': 'N/A',
                    'error': 'No invoices extracted'
                })
                continue
