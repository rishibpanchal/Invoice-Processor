...
def launch_gui():
    ...
    gui_script = Path(__file__).parent / "qt viewer" / "main_qt.py"
    
    if not gui_script.exists():
        print(f"❌ GUI script not found: {gui_script}")
        return False
    
    print("🚀 Launching Invoice Processing GUI...")
    try:
...
def test_extraction():
    """Test the extraction functionality with a sample file"""
    files_dir = Path(__file__).parent / "files"
    pdf_files = list(files_dir.glob("*.pdf"))
    
    if not pdf_files:
        print("ℹ️ No PDF files found for testing")
        return True
    
    print(f"🧪 Testing extraction with {pdf_files[0].name}...")
    
    try:
        # Import and test the enhanced extractor
        ...
        result = extractor.extract_data(str(pdf_files[0]))
        
        if result.get('error', False):
            print(f"⚠️ Extraction test failed: {result.get('error_message', 'Unknown error')}")
            return False
        else:
            summary = extractor.get_extraction_summary(result)
            print(f"✅ Extraction test successful: {summary['fields_extracted']} fields extracted")
            return True
...
def show_help():
    ...
    print()
    print("Usage:")
    print("1. Run this script to launch the GUI")
    print("2. Click 'Load PDF' to select an invoice")
    print("3. Click 'Process PDF' to extract data")
...
def main():
    ...
    response = input("Do you want to launch the GUI now? (y/n): ").lower().strip()
    if response in ['y', 'yes', '']:
        if not launch_gui():
            print("❌ Failed to launch application")
            sys.exit(1)
    else:
        print("ℹ️ Launch cancelled by user")
        print("To launch manually, run: python 'qt viewer/main_qt.py'")
